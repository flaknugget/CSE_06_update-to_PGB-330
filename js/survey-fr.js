// JavaScript Document
			...
			function outputQuizForm() {
											...
											var quizEmail = $("#quizEmail").val();
																						
										
											var quizRequestedDocs = $("[name=quizCheckbox]:checked").map(function() {return this.value;}).get().join("  /  ");
											
											
											if (typeof quizName==='undefined') {
												quizName = "&nbsp;";
											}		
											if (typeof quizEmail==='undefined') {
												quizEmail = "&nbsp;";
											}		
											
											if(quizRequestedDocs.length == 0 ) {
												var quizRequestedDocs = "No Documents Selected";
											}
											
											
											output = "<tr><td>"+datetime+"</td><td>"+language+"</td><td>"+quizName+"</td><td>"+quizEmail+"</td><td>"+quizRequestedDocs+"</td></tr>";
			...
			function outputForm() {
											...
											if (q01WorkSectorOther.match(/\S/) && q01workSector=="Autre") {
												q01workSector = q01workSector + ' : ' + q01WorkSectorOther;
											}
																
											if (q01WorkSectorIndustry.match(/\S/) && q01workSector=="Industrie") {
											...
											if (q02workSectorOther.match(/\S/) && q02workField=="Autre") {
											...
											if (q03workPositionOther.match(/\S/) && q03workPosition=="Autre") {
											...
											if (q08usefulPubsWhy.match(/\S/) && q08usefulPubs=="Non") {
						...
						function gotFS(fileSystem) {
							...
							var fileName = "survey-results_"+todaysDate.getDate()+"-"+(todaysDate.getMonth()+1)+"-"+todaysDate.getFullYear()+".txt";		
							
							$("#output").append("<br />"+fileName);
							
							fileSystem.root.getFile(fileName, {create: true, exclusive: false}, gotFileEntry, fail);
							// fileSystem.root.getFile("surveyOutput-Aug22.txt", {create: false}, gotFileEntry, createFile);
						
							$("#output").append("<br />gotFS()");
						}
						
						function gotFSquiz(fileSystem) {
							var todaysDate = new Date(); 
							var fileName = "information-requests_"+todaysDate.getDate()+"-"+(todaysDate.getMonth()+1)+"-"+todaysDate.getFullYear()+".txt";		
